...
class MainFrame(wx.Frame):
	def __init__(self, *args, **kwds):
		...
		item = menu.Append(wx.NewId(), _("mapCutr User &Group"))
		...
		self.label_10 = wx.StaticText(self, -1, _("mapCutr - Tile Generator for Map Mashups"))
		
		self.label_8 = wx.StaticText(self, -1, _("mapCutr 1.0 alpha"))
		self.label_9 = wx.StaticText(self, -1, _(u"build on MapTiler <3"))
	...
	def OnAbout(self, event):
		...
		info.Copyright = u"build on MapTiler / developed by the community <3"
		info.Description = _("""mapCutr is a powerful tool for online map publishing and generation of raster overlay mashups.
Your geodata are transformed to the tiles compatible with Google Maps and Earth - ready for uploading to your webserver.""")
	...
	def OnProjectWeb(self, event):
		webbrowser.open_new(_("https://github.com/stromberger/mapCutr"))
	...
	def OnOpen(self, event):
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			if len(bad_paths) > 0:
				wx.MessageBox(_("mapCutr doesn't have permission to read the following files:\n\n") + "\n".join(bad_paths),
...
because its superdirectory '%s' is not writeable.""") % (config.outputdir, dirname),
	...
	def _add(self, filename):
		...
		if len(config.files) > 0:
			wx.MessageBox(_("""Unfortunately the merging of files is not yet implemented in the mapCutr GUI. Only the first file in the list is going to be rendered."""), _("Not yet implemented :-("), wx.ICON_ERROR)
